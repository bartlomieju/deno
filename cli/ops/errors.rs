...
pub fn init(rt: &mut deno_core::JsRuntime) {
  ...
  super::reg_json_sync(rt, "op_apply_source_maps", op_apply_source_maps);
...
#[serde(rename_all = "camelCase")]
struct ApplySourceMap2 {
  file_name: Option<String>,
  line_number: Option<i32>,
  column_number: Option<i32>,
}

fn op_apply_source_maps(
  state: &mut OpState,
  args: Value,
  _zero_copy: &mut [ZeroCopyBuf],
) -> Result<Value, AnyError> {
  let args: Vec<ApplySourceMap2> = serde_json::from_value(args)?;

  let mut mappings_map: CachedMaps = HashMap::new();
  let program_state = state.borrow::<Arc<ProgramState>>().clone();

  let response: Vec<Value> = args.into_iter().map(|arg| {
    if arg.file_name.is_none() | arg.line_number.is_none() | arg.column_number.is_none() {
      return serde_json::to_value(arg).unwrap();
    }

    let (orig_file_name, orig_line_number, orig_column_number, _) = get_orig_position(
      arg.file_name.unwrap(),
      arg.line_number.unwrap().into(),
      arg.column_number.unwrap().into(),
      None,
      &mut mappings_map,
      program_state.clone(),
    );
